// -----------------------------------------------------------------------------------------
// Program:  ESPVideoCamera 
// Summary:  T5 capacitor touch pin trigger video camera that saves AVI files to SD card.
// Author:   JasonZ
//           
...
//  PIR sensor is connected to GPIO12(T5).  This is the RX for the serial connection.  The sensor should be disconnected when uploding the sketch to avoid issues.
...
const long unsigned MOTION_DELAY    = 25000; // Time (ms) after motion last detected that we keep recording
...
void setup()
{
  ...
  pinMode(33, OUTPUT);
  pinMode(4, OUTPUT);
  digitalWrite(33, HIGH);   // turn the LED off (HIGH is the voltage level)
...
void initialiseCamera()
{
  ...
  config.frame_size   = FRAMESIZE_VGA;  //  800 x  600. 40ms capture.
...
void startFile()
{ 
  char AVIFilename[30] = "/sdcard/VID";
